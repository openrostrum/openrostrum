...
| Visual regression tooling (Storybook, screenshot diffing) | Needs a baseline skin to diff against; the skin is deliberately a neutral stand-in until the owner designs it (`docs/engineering.md` → Design system). Re-scope once a real skin exists. |
| ~~Multi-org~~, Event Team roles/permissions matrix | **Multi-org is now IN — P1 #22 (owner decision 2026-08-10: real organizer sign-up on the flagship deployment; do not relitigate)**; multiple equal admin accounts are P1 #21; only the roles/permissions MATRIX stays out (parity-verified: Sessionboard's custom-role toggle matrix, not its equal-admins core) |
