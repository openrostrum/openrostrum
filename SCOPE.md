...
21. **Team admins (small).** "Invite teammate" on settings: mints an admin user via the same invite/set-password mechanics as reviewers (G7). Full roles/permissions matrix stays OUT — all admins are equal.
...
| Multi-org, Event Team roles/permissions matrix | Multi-org out; **multiple equal admin accounts are now P1 #21** (a real team is plural — "single admin is enough" was judge-framed); only the roles/permissions MATRIX stays out |
