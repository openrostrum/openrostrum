...
**Sources of truth:** video walkthrough (transcript + frames in `docs/reference/`), Google Doc brief **PDF** (`docs/reference/brief.pdf` — the PDF matters: it shows strikethroughs and red priority callouts that plain text hides), Discord for clarifications. **Requirements froze with the Aug 9 eval-kit release** — the vendored kit (`docs/reference/killmysaas-evals/`) is the scoring truth; where Discord and the kit conflict, the kit wins.
