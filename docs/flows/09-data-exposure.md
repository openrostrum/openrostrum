...
| **API token consumer** | `x-access-token` (org-generated, scoped, Hide-PII flag, event restrictions) or OAuth 2.1 read-only ([08 §2e](08-settings-data-api.md)) | 177-op public API; webhooks (push of full resource objects) | P2 #4 |
| **Public site visitor via embeds** | none | 5 embed widgets + JSON/XML/iCal feeds; deep links `?sb-session-id=`, `?sb-speaker-id=` ([06 §2d](06-agenda-embeds.md)) | P2 #2 |
...
| q | **Export file-field transform** | Module exports render file-type fields as **publicly hosted URL links** — exporting is an act of publishing those file URLs. | https://learn.sessionboard.com/reporting/exporting-data; [08 §2c](08-settings-data-api.md) | P2 #3 |
...
6. **API without the PII/scope layer.** Shipping the P2 API without Hide-PII default-on, event restrictions, and per-scope write checks turns any leaked token into a full CRM dump; also remember OAuth-style tokens must never write. (Rule p.)
