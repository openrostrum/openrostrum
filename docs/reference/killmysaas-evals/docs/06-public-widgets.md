...
- **EMB-14** (w3, auto): **distribution to non-admin viewers** — not anonymity — is the defining property of the five widgets. CALIBRATION NOTE (2026-08-10): this item originally required login-free public reachability. Testing the harness against SessionBoard itself disproved that premise: SessionBoard's event site has no public option — Site > Edit Login Options shows `Standard Login [checked] [disabled]` with the copy "At least one login method must be enabled" — so an agent touring the app the way EMB-S1 does would hit an auth wall and the reference product would have failed its own rubric on 20% of required weight. (The nuance: SessionBoard's *embed script* is genuinely anonymous once pasted onto a third-party marketing page — it is the *event site/portal* that cannot be made public. The old item conflated the two.) The item now asks whether each widget renders populated content outside the admin UI to a viewer with no organizer rights (anonymous page, attendee-authenticated site/portal, or a generated embed). Anonymous access is noted as a bonus; admin-only widgets are the failure mode. Verified across EMB-S1/EMB-S2 (non-admin viewer) plus the rendered embed from EMB-S3.
- **EMB-15** (w3, auto-partial): the organizer-side generator (types, formats, branding, filters, fields, per-embed Get Code) is the **primary evidence that the widgets are genuinely embeddable**, which is why it was raised from w2 when EMB-14 stopped grading anonymity. SessionBoard's live surface is CMS > Embeds ("Export a feed of your agenda, sessions, or speakers to place in your app or website") with named embeds, a format grouping, enable/disable state, and a per-embed Get Code action. The agent must capture the snippet text verbatim, not merely assert a builder exists; real third-party embedding and feed contents still need a human — hence auto-partial.
- **EMB-16** (w3, auto-partial): live-data consistency (no republish drift) is the widgets' value proposition; cross-widget and organizer-vs-public sampling gives an objectively judgeable point-in-time auto half, but edit-propagation to an already-embedded widget without republishing (and the ~60-min refresh cadence) is time-delayed and needs a human — hence the manual instructions.
