...
| L4 | `/onboarding` keeps its own event form + timezone machinery; the shared `app/settings/event-form` covers settings/create-event only (onboarding now shares the slug-taken detection) | CLOSED (onboarding-first-run lane, 2026-08-11; critic-shareable, 2026-08-13) — first run is still three short steps and does not reuse the full event-details form. Timezone lives in `app/components/timezone-select.tsx`. Step 1 now shows the derived slug and validates uniqueness with `SLUG_TAKEN_MESSAGE` instead of minting a silent `-2` |
