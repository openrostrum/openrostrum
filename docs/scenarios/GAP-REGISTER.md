...
| C2 | **Speaker-CRM staging files await owner adoption at merge** — `app/features/contacts/textarea.tsx` → move verbatim into `app/ui` (multi-line Input counterpart; token-for-token recipe); `app/features/contacts/confirm-dialog.tsx` → move verbatim into `app/ui` (the repo's first in-app confirm dialog — K7 bans native confirm(), so every destructive flow will need it); `app/features/contacts/status.ts` → fold `CONTACT_STATUS_TONE` into `app/ui/status-badge.tsx`; `app/db/schema.ts` `CONTACT_STATUS` → fold onto the `app/db/constants` import (SUBMISSION_STATUS precedent; lockstep test pins the tuples until then). `app/ui`/`schema.ts` are hook-guarded, so the lane cannot land these itself; `app/features/contacts/` must be EMPTY and deleted once adopted. | OPEN — integration owner, same unit of work as the roster-lane merge |
