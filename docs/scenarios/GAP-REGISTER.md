...
| L4 | `/onboarding` keeps its own event form + timezone machinery; the shared `app/settings/event-form` covers settings/create-event only (onboarding now shares the slug-taken detection) | CLOSED (onboarding-first-run lane, 2026-08-11; public-site landing, 2026-08-13) — first run is one naming screen and does not reuse the full event-details form. Dates, location, and timezone stay in `/admin/settings`. The name step shows the derived slug as `/schedule/{slug}` and validates uniqueness with `SLUG_TAKEN_MESSAGE` instead of minting a silent `-2` |
...
## Bug-hunt lane deferrals (2026-08-14) — UI-only fixes, ST1 class

Both fixes in PR `fix/bug-hunt-noops` are UI-only regressions under the ST1
ruling above: the workerd suite runs loaders/actions only, so the client wiring
each bug lived in cannot be pinned in-harness. Recorded live-browser
verification per ST1's OPEN escalation, pending the owner's render-harness
decision; each is the reproduction the harness must pin when it lands.

| # | Deferral | Status |
|---|----------|--------|
| BH1 | **Evaluation list "Delete plan" confirm closed without submitting** — the confirm's submit button carried `onClick={() => setDeleting(null)}`; the synchronous state flush unmounted the inline `<Form>` before the browser's default submit action ran, so no `intent=delete-plan` POST ever fired and the plan stayed (server action itself was and remains covered by `test/admin.evaluation.route.test.ts`). Reproduced live pre-fix (click → panel closes, zero network); post-fix the POST fires and the plan disappears. Fix removes only the competing onClick — the panel closes via the re-render once the action drops the plan from the derived `deletingPlan`. | OPEN (owner: render harness, per ST1) |
| BH2 | **Bare `/cfp` alias hijacked every `/cfp/:eventSlug`** — fs-routes nested `cfp.$eventSlug.tsx` under the same-prefix bare `cfp.tsx`, so the bare alias's layout loader ran on every per-event URL and its redirect to the DEFAULT event's oldest open form won over the child's 404/empty-state/own-form answer. `#146`'s shareable per-event CFP links misrouted every non-default event's speakers to the default event's form. Fixed by renaming `cfp.tsx` → `cfp._index.tsx` (index routes are never parents; the same convention the sessions/speakers/schedule/gallery alias families already use). The loader-level unit tests cannot see the fs-routes manifest topology (workerd cannot load `app/routes.ts` or the host filesystem), so a future bare-alias-parent regression has no in-harness oracle. Verified live pre/post: `/cfp/bogus` 302→default form pre, 404 post; `/cfp/deep-dive-2026` 302→default form pre, Deep Dive's designed empty state post. | OPEN (owner: render harness + optional route-topology lint, per ST1) |
