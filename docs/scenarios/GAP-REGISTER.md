...
| L4 | `/onboarding` keeps its own event form + timezone machinery; the shared `app/settings/event-form` covers settings/create-event only (onboarding now shares the slug-taken detection) | CLOSED (onboarding-first-run lane, 2026-08-11) — resolved by deletion rather than convergence: first run is now three one-question steps and carries no event form, so there is nothing left to converge. The timezone machinery that WAS duplicated is now one component, `app/components/timezone-select.tsx`, used by both first run and `app/settings/event-form.tsx`; slug is derived server-side (`app/domain/onboarding.ts`) instead of asked |
