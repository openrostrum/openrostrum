...
| CFP entry alias (redirect) | `cfp._index.tsx` (`_index`, not bare, so fs-routes does not nest `cfp.$eventSlug.tsx` under it) | `/cfp` → the default event's oldest open submission form at `/submit/:eventSlug/:formId` (the homepage "Call for speakers" link — stable URL over the per-form uuid; no open form → `/`; `// @public`) | 2 | done |
