...
| Portals admin | `admin.portals.tsx` (list + "View portal as" preview) · `admin.portal-forms.tsx` (portal-form builder) | `/admin/portals` · `/admin/portal-forms` | 3 | done (file requests live on task definitions via `isFileRequest` — no separate route) |
