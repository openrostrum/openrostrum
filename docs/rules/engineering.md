...
- The skin is **locked**: "Gallery" — full contract (tokens, type, grid, states, the petrol law) in [`design-system.md`](design-system.md). Colors resolve via `light-dark()` — components **never write `dark:` variants, no exceptions** [lint-enforced: `no-dark-variants`]: theme = `color-scheme` on `<html>` (cookie-persisted, tri-state), and a media-query variant desyncs the moment a visitor overrides the OS (even `StatusBadge`'s conventional status hues are `light-dark()` pairs). Re-skinning edits tokens + `app/ui` **with zero route diffs** — that property is the point, keep it true.
