...
Same-repo PRs that pass the `quality` job get an isolated Cloudflare preview (`scripts/preview/`, `pnpm preview:deploy` / `pnpm preview:teardown`). The production `deploy` job on `main` is unchanged: it still migrates `openrostrum` and deploys worker `openrostrum` to `openrostrum.com`. Preview scripts refuse those production names, but the API token is still account-scoped Edit — isolation is the scripts, not Cloudflare resource ACLs. The `preview` job is advisory: it must not be a required merge check. A failed or skipped teardown can leave `openrostrum-pr-<n>` resources until the next close or a manual `pnpm preview:teardown`. Secrets and token permissions live in `.deploy.env.example`; grader-facing URL shape is in [`JUDGING.md`](../JUDGING.md).
