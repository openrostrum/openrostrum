...
// itself carries no deferral marker (see docs/rules/engineering.md → No shortcuts).
const DEFERRAL =
...
export const noDeferralComments = {
	meta: {
		...
		messages: {
			deferral:
				'Deferral marker in a comment ("{{match}}"). Build it right, or raise it: a deferral is a scope decision for the integration owner (it becomes a SCOPE row) — then delete this marker; an approved unbuilt path throws, it never silently degrades. See docs/rules/engineering.md → No shortcuts.',
		},
	},
	create(context) {
		return {
